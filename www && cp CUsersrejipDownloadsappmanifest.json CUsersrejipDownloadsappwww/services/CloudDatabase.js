...
class CloudDatabase {
  ...
  async init(sampleData, demoCredentials) {
    ...
    try {
      ...
      let users = [];
      if (demoCredentials) {
        ['students', 'teachers', 'parents', 'admins'].forEach(role => {
          if (demoCredentials[role]) {
            const formattedRole = role === 'students' ? 'student' : 
                                (role === 'teachers' ? 'teacher' : 
                                (role === 'parents' ? 'parent' : 'admin'));
  ...
  async getAllUsers() {
    if (!this.isConfigured) return this.localDB.data.users || [];
    const snapshot = await this.db.collection('users').get();
    return snapshot.docs.map(d => d.data());
  }

  async getAllCourses() {
    if (!this.isConfigured) return this.localDB.data.courses || [];
    // Currently courses might be in 'global/courses' or 'courses' collection
  ...
  async getAllAssignments() {
    if (!this.isConfigured) return this.localDB.data.assignments || [];
    const doc = await this.db.collection('global').doc('assignments').get();
    if (doc.exists && doc.data().data) return doc.data().data;
    const snapshot = await this.db.collection('assignments').get();
    return snapshot.docs.map(d => d.data());
  }

  async createUser(userData) {
    if (!this.isConfigured) {
      const users = this.localDB.data.users || [];
      users.push(userData);
      this.localDB.data.users = users;
      localStorage.setItem('learnora_users', JSON.stringify(users));
      return userData;
    }
  ...
  async createCourse(courseData) {
    if (!this.isConfigured) {
      const courses = this.localDB.data.courses || [];
      courses.push(courseData);
      this.localDB.data.courses = courses;
      localStorage.setItem('learnora_courses', JSON.stringify(courses));
      return courseData;
    }
    await this.db.collection('courses').doc(courseData.id).set(courseData);
    return courseData;
  }

  async createParentStudentConnection(connData) {
    const connId = `conn_${Date.now()}`;
    const connection = { ...connData, id: connId };
    if (!this.isConfigured) {
      const connections = this.localDB.data.userConnections || [];
      connections.push(connection);
      this.localDB.data.userConnections = connections;
      localStorage.setItem('learnora_connections', JSON.stringify(connections));
      return connection;
    }
    await this.db.collection('parentStudentConnections').doc(connId).set(connection);
    return connection;
  }

  async getParentStudentConnections() {
    if (!this.isConfigured) {
      return this.localDB.data.userConnections || [];
    }
    const snapshot = await this.db.collection('parentStudentConnections').get();
    return snapshot.docs.map(d => d.data());
  }

  async deleteUser(userId) {
    if (!this.isConfigured) {
      const users = (this.localDB.data.users || []).filter(u => u.id !== userId);
      this.localDB.data.users = users;
      localStorage.setItem('learnora_users', JSON.stringify(users));
      return;
    }
  ...
  async addAssignment(assignment) {
    if (!this.isConfigured) {
      this.localDB.data.assignments = this.localDB.data.assignments || [];
      const newAssignment = { id: 'assign_' + Date.now(), ...assignment, createdAt: new Date().toISOString() };
      this.localDB.data.assignments.push(newAssignment);
      this.localDB.saveData();
      return newAssignment;
    }

    try {
      const assignmentRef = this.db.collection('assignments').doc();
      const newAssignment = { 
        id: assignmentRef.id, 
        ...assignment, 
      ...
      // Also update global assignments for backward compatibility if needed
      const doc = await this.db.collection('global').doc('assignments').get();
      if (doc.exists) {
        const data = doc.data().data || [];
        data.push(newAssignment);
        await this.db.collection('global').doc('assignments').update({ data });
      }
  ...
  async sendMessage(senderId, receiverId, content) {
    if (!this.isConfigured) {
      this.localDB.data.messages = this.localDB.data.messages || [];
      const newMsg = { id: 'msg_' + Date.now(), senderId, receiverId, content, timestamp: new Date().toISOString() };
      this.localDB.data.messages.push(newMsg);
      this.localDB.saveData();
      return newMsg;
    }
  ...
  async getMessagesForUser(userId) {
    if (!this.isConfigured) {
      const msgs = this.localDB.data.messages || [];
      return msgs.filter(m => m.receiverId === userId || m.senderId === userId)
                 .sort((a,b) => new Date(a.timestamp) - new Date(b.timestamp));
    }

    try {
      // Because we want OR (sender or receiver), we do 2 queries and merge to avoid compound index errors
      const sentSnapshot = await this.db.collection('messages').where('senderId', '==', userId).get();
      const receivedSnapshot = await this.db.collection('messages').where('receiverId', '==', userId).get();
      
      const messages = [];
      sentSnapshot.forEach(d => messages.push(d.data()));
      receivedSnapshot.forEach(d => {
        // Deduplicate if needed, though they shouldn't overlap here
        if (!messages.find(m => m.id === d.data().id)) {
