function LearnoraApp() {
  ...
  // Ensure Kiosk Mode is active for Students
  React.useEffect(() => {
    const checkKiosk = async () => {
      if (window.NativePlugins && isAuthenticated && userRole === 'student') {
        try {
          const status = await window.NativePlugins.KioskMode.isActive();
          if (!status.active) {
            await window.NativePlugins.KioskMode.enable();
          }
        } catch (e) {
          console.error('[KIOSK] Failed to verify kiosk status', e);
        }
      }
    };
    
    const interval = setInterval(checkKiosk, 5000);
    checkKiosk();
